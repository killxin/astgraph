...
public class ASTViewContentProvider {
	...
	public Object[] getChildren(Object parent) {
		if (parent instanceof ASTAttribute) {
			...
			if (isPrint == false) {
				...
				File tfile = new File(out[out.length - 2] + ".txt");
				File gfile = new File(out[out.length - 2] + ".gv");
				PrintWriter toutput = null;
				PrintWriter goutput = null;
				try {
					toutput = new PrintWriter(tfile);
					goutput = new PrintWriter(gfile);
				} catch (FileNotFoundException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}

				goutput.println("digraph G {");
				goutput.println("node [shape=\"record\"]");
				String[] sout = out[0].split("\\\\");
				goutput.println("struct" + Driver.NO + "[label=\""
						+ sout[sout.length - 1] + "\"]" + ";");
				Driver.NO++;
				...
				for (int i = 0; i < obj.length; i++) {
					if (obj[i] instanceof NodeProperty) {
						...
						try {
							int x = Driver.NO;
							dfsDisplayProperty(node, 0, toutput, goutput);
							goutput.println("struct0" + "->" + "struct" + x
									+ ";");
							Driver.NO++;
	...
	public void dfsDisplayProperty(NodeProperty node, int level,
			PrintWriter toutput, PrintWriter goutput)
		...
		int p = Driver.NO;
		label = str.replace('>', 'm').replace('<', 'l').replace('"', '\'');
		goutput.println("struct" + p + "[label=\"" + label + "\"]" + ";");
		...
		for (Object o : ob) {
			if (o instanceof ASTNode) {
				...
				toutput.println(name[name.length - 1]);
				Driver.NO++;
				int q = Driver.NO;
				label = name[name.length - 1].replace('>', 'm')
						.replace('<', 'l').replace('"', '\'');
				goutput.println("struct" + q + "[label=\"" + label + "\"]"
						+ ";");
				...
				for (int i = 0; i < obj.length; i++) {
					if (obj[i] instanceof NodeProperty) {
						...
						Driver.NO++;
						int c = Driver.NO;
						dfsDisplayProperty(n, level + 1, toutput, goutput);
						goutput.println("struct" + q + "->" + "struct" + c
								+ ";");
					}
				}

				goutput.println("struct" + p + "->" + "struct" + q + ";");
			}
		}
	...
	private Object[] getNodeChildren(ASTNode node) {
		...
		if (node instanceof Expression) {
			...
			if (expression instanceof Name) {
				...
				try {
					IMethodBinding binding = ((LambdaExpression) expression)
							.resolveMethodBinding();
					bindingAttribute = createBinding(expression, binding);
				} catch (RuntimeException e) {
					bindingAttribute = new Error(res, ">binding: Error: "
							+ e.getMessage(), e);
				}
				res.add(bindingAttribute);
			} else if (expression instanceof MethodReference) {
				IMethodBinding binding = ((MethodReference) expression)
						.resolveMethodBinding();
				res.add(createBinding(expression, binding));
			}
		...
		List<StructuralPropertyDescriptor> list = node
	...
	private Object createExpressionTypeBinding(ASTNode parent,
